    ...
    for i in "${!logo2[@]}"; do
        ...
        case "$i" in
            ...
            6)
                ...
                ;;
            7)
                out+="${fgpur}Temperature........: CPU: ${cpuTempC}°C/${cpuTempF}°F GPU: ${gpuTempC}°C/${gpuTempF}°F"
                ;;
            8)
                out+="${fgpur}Free Disk Space....: `df -Ph | grep -E '^/dev/root' | awk '{ print $4 " of " $2 }'`"
                ;;
            9)
                out+=""
                ;;
            10)
                out+="${fgwht}The Frambox Project, https://geheimesite.nl"
                ;;
        esac
        out+="${rst}\n"
    done
